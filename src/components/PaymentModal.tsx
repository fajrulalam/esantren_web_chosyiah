...
export default function PaymentModal({
  ...
  const uploadPaymentProof = async (
    ...
    // Use the santri name from paymentData (not from auth context)
    // This ensures we use the correct santri name even when admin uploads
    // Field name in PaymentStatus is 'santriName' not 'nama'
    const actualSantriName =
      (paymentData as any)?.santriName ||
      paymentData?.nama ||
      santriName ||
      "unknown_santri";
    const sanitizedSantriName = actualSantriName
      .replace(/\s+/g, "_")
      .toLowerCase();

    // Extract payment name from invoiceId (format: "YYYY-MM-DD_Name_Details_timestamp")
    // Or use a generic name if not available
    let actualPaymentName = "payment";
    if (paymentData?.invoiceId) {
      // Extract the readable part from invoiceId
      // Example: "2025-04-02_Syahriah_Semester_Genap_2024-2025_1743597509614"
      const parts = paymentData.invoiceId.split("_");
      // Remove timestamp (last part) and date (first part), join the rest
      if (parts.length > 2) {
        actualPaymentName = parts.slice(1, -1).join("_");
      }
    }
    const sanitizedPaymentName = actualPaymentName
      .replace(/\s+/g, "_")
      .toLowerCase();
    ...
    const storageRef = ref(
      storage,
      `payment_proofs/${sanitizedSantriName}/${sanitizedPaymentName}_${timestamp}.${fileExtension}`
    );
    ...
    // Upload the file with metadata to ensure correct content type
    const metadata = {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      setError(
        `Terjadi kesalahan saat memproses pembayaran: ${errorMessage}. Silakan coba lagi.`
      );
      setLoading(false);
    }
  };
  ...
  return (
    <div className={containerClass}>
      ...
      <div
        className={
          isMobile
            ? "w-full rounded-t-xl shadow-lg bg-gray-50 dark:bg-gray-900 z-30"
            : "modal-content rounded-lg shadow-lg bg-gray-50 dark:bg-gray-900 z-30"
        }
      ...
      >
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          {!isMobile && (
            <button
              onClick={resetAndClose}
              className="text-text-light dark:text-gray-400 hover:text-gray-800 dark:hover:text-white"
            >
              ✕
            </button>
          )}
