...
export default function Login() {
    ...
    const { user, signInWithEmail, signInWithGoogle, signInAsSantri, loading, checkSantriName, checkSantriPhone } = useAuth();
    ...
    const handleWaliSantriLogin = async () => {
        ...
        try {
            ...
            if (!isNameFound) {
                // Name doesn't exist
                setError('Nama santri tidak ditemukan. Mohon periksa kembali penulisan nama.');
                setPhoneCorrect(null);
            } else {
                // Name exists but phone doesn't match
                setPhoneCorrect(false);
                setError('Nama santri ditemukan, tetapi nomor telepon tidak sesuai dengan data yang terdaftar.');
            }
        } catch (err) {
    ...
    const handleReportPhoneNumber = () => {
        ...
        const message = encodeURIComponent(
            `Assalamu'alaikum, Saya ${namaSantri} tidak bisa masuk website Esantren Chosyiah meskipun menurut saya nomor sudah sesuai. Mohon bantuannya untuk mengecek / memperbarui data saya dengan nomor ini.`
        );
    ...
    return (
        <div className="flex justify-center items-center min-h-screen bg-amber-50 dark:bg-gray-900 pt-24 px-4">
            <div className={`${containerStyle} w-full max-w-md`}>
                ...
                {error && !(nameFound === false || (nameFound === true && phoneCorrect === false)) && (
                        ...
                        <>
                            <div className="mb-4">
                                ...
                                <div className="relative">
                                    <input
                                        type="text"
                                        className={`${inputStyle} ${nameFound === true ? 'border-green-400 pr-10' : nameFound === false ? 'border-red-400 pr-10' : ''}`}
                                        value={namaSantri}
                                        onChange={(e) => {
                                    ...
                                    />
                                    {nameFound === true && (
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-green-500 absolute right-3 top-1/2 transform -translate-y-1/2">
                                            <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd" />
                                        </svg>
                                    )}
                                    {nameFound === false && (
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-red-500 absolute right-3 top-1/2 transform -translate-y-1/2">
                                ...
                                <div className="mt-1 space-y-1">
                                    <p className="text-xs text-amber-600">
                                        <span className="font-medium">Penting:</span> Pastikan penulisan nama sudah benar.
                                    </p>
                                    <p className="text-xs text-amber-700">
                                        Contoh format yang benar: <span className="font-medium">Ahmad Baihaqi</span>, <span className="font-medium">M. Fajrul Alam</span>
                                    </p>
                                    <p className="text-xs text-amber-700">
                                ...
                                {nameFound === false && (
                                    <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
                                        <div className="flex items-start">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-red-500 mt-0.5 mr-2 flex-shrink-0">
                                                <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zM12 8.25a.75.75 0 01.75.75v3.75a.75.75 0 01-1.5 0V9a.75.75 0 01.75-.75zm0 8.25a.75.75 0 100-1.5.75.75 0 000 1.5z" clipRule="evenodd" />
                                            </svg>
                                            <div>
                                                <p className="font-medium">Nama tidak ditemukan</p>
                                                <ul className="list-disc list-inside text-xs mt-1 space-y-1">
                                                    <li>Periksa kembali ejaan nama (sistem akan memperbaiki kapitalisasi)</li>
                                                    <li>Gunakan nama lengkap seperti yang terdaftar saat pendaftaran</li>
                                                    <li>Jika menggunakan inisial, pastikan format dengan titik (contoh: M. Ahmad)</li>
                                                    <li>Jika masih gagal, coba tanpa inisial atau menggunakan nama panggilan</li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex relative">
                                    ...
                                    <input
                                        type="text"
                                        className={`${inputStyle} rounded-l-none border-l-0 ${phoneCorrect === true ? 'border-green-400 pr-10' : phoneCorrect === false ? 'border-red-400 pr-10' : ''}`}
                                        value={phoneNumber}
                                        onChange={(e) => {
                                            // Only allow digits
                                ...
                                {nameFound === true && phoneCorrect === false && (
                                    <div className="mt-3">
                                        <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800 mb-3">
                                            <div className="flex items-start">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-yellow-500 mt-0.5 mr-2 flex-shrink-0">
                                                    <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zM12 8.25a.75.75 0 01.75.75v3.75a.75.75 0 01-1.5 0V9a.75.75 0 01.75-.75zm0 8.25a.75.75 0 100-1.5.75.75 0 000 1.5z" clipRule="evenodd" />
                                                </svg>
                                                <div>
                                                    <p className="font-medium">Nama santri ditemukan, tetapi nomor telepon tidak sesuai</p>
                                                    <p className="text-xs mt-1">Periksa kembali nomor yang Anda masukkan. Jika Anda yakin nomor tersebut benar, klik tombol di bawah untuk menghubungi kami via WhatsApp.</p>
                                                </div>
                                            </div>
                                        </div>
                ...
                <div className="mt-8 text-center">
                    <Link href="/" className="text-amber-700 dark:text-amber-400 hover:text-amber-900 dark:hover:text-amber-300 transition-colors">
                        ← Kembali ke Beranda
                    </Link>
                </div>
            </div>
