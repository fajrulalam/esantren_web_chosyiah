    ...
    const fetchUsers = async () => {
      try {
        ...
        const usersList = querySnapshot.docs
          ...
          .filter((user) => user.kodeAsrama === KODE_ASRAMA); // Filter users by KODE_ASRAMA
        setUsers(usersList);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createNewUser({
        ...
        phoneNumber,
        honoraryPronoun,
        kodeAsrama: KODE_ASRAMA, // Use constant instead of state
        namaPanggilan,
        tanggalLahir,
      });
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm text-yellow-700 dark:text-yellow-200">
              Important: Users will need to sign in with their Google account
              using the email address you provide here. Their access will be
              determined by the role you've assigned.
            </p>
          </div>
        </div>
      </div>
