...
function RekapitulasiViewContent() {
  ...
  // Log clearly
  console.log("Rekapitulasi View Page loaded with: ", {
    paymentId,
    paymentName,
  });
      ...
      if (invoiceDoc.exists()) {
        ...
        setInvoiceDetails({
          ...
          numberOfWaitingVerification:
            invoiceData.numberOfWaitingVerification || 0,
  ...
  const openWhatsAppWithApprovalMessage = (
    ...
    if (isPartialPaymentType) {
      ...
      message = `[Pembayaran Sebagian Terverifikasi]\n\n*Assalamu'alaikum Wali Santri dari Ananda ${
        payment.nama
      }*,\n\n*Alhamdulillah!* Pembayaran sebagian untuk *${paymentName}* sebesar *${formatCurrency(
        amount
      )}* telah *berhasil diverifikasi*.\n\nRincian pembayaran:\n• Jumlah dibayarkan: *${formatCurrency(
        amount
      )}*\n• Total tagihan: *${formatCurrency(
        payment.total
      )}*\n• Sisa yang harus dibayar: *${formatCurrency(
        payment.total - payment.paid
      ...
      message = `[Pembayaran Terverifikasi]\n\n*Assalamu'alaikum Wali Santri dari Ananda ${
        payment.nama
      }*,\n\n*Alhamdulillah!* Pembayaran untuk *${paymentName}* sebesar *${formatCurrency(
        amount
      )}* telah *berhasil diverifikasi*.\n\nRincian pembayaran saat ini:\n• Total dibayarkan: *${formatCurrency(
        payment.paid
      )}*\n• Total tagihan: *${formatCurrency(
        payment.total
      )}*\n• Sisa yang perlu dilunasi: *${formatCurrency(
        payment.total - payment.paid
