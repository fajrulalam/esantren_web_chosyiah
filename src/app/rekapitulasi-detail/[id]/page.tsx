...
export default function PaymentDetailPage() {
  ...
  // Get base64 encoded ID from params and decode it
  let paymentId = '';
  if (params.id && typeof params.id === 'string') {
    try {
      // Decode the base64 string to get the original payment ID
      paymentId = atob(params.id);
      console.log("Decoded payment ID:", paymentId);
    } catch (error) {
      console.error("Error decoding payment ID:", error);
      paymentId = params.id;
    }
  } else {
  ...
  console.log("Detail page mounted with paymentId:", paymentId);
  ...
  // Call fetchSantriPaymentStatus when paymentId is available
  useEffect(() => {
    if (paymentId) {
