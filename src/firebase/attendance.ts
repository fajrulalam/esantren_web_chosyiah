...
export async function createAttendanceSession(
  ...
  if (attendanceTypeId) {
    console.log(
      `Creating session from attendance type ID: ${attendanceTypeId}`
    );

    // Get the attendance type document to check for specific santri IDs
    const attendanceTypeDoc = await getDoc(
      doc(db, "AttendanceTypes", attendanceTypeId)
    );
    ...
    if (attendanceTypeDoc.exists()) {
      ...
      console.log(
        `Attendance type has ${listOfSantriIds.length} specified santris`
      );
      ...
      console.log(
        "Attendance type ID provided but document not found, using default behavior"
      );
      await addActiveStudents();
    }
  } else {
    ...
    console.log(
      "Manual input session (not from attendance type selection), using default behavior"
    );
    await addActiveStudents();
  }
  ...
  async function addActiveStudents() {
    ...
    console.log(
      `Added ${studentSnapshot.docs.length} active students with appropriate statuses`
    );
  }
...
export async function loadStudentsForDormitory(
  kodeAsrama: string
): Promise<SantriWithAttendance[]> {
  const q = query(
    collection(db, "SantriCollection"),
    where("kodeAsrama", "==", kodeAsrama)
  );
...
export async function markAttendance(
  ...
  console.log(
    `Marking student ${santriId} as ${status} in session ${sessionId}`
  );
...
export function findLateReturnStudents(
...
export async function generateAttendanceReport(
  ...
  let records = recordsSnapshot.docs.map(
  ...
  if (filters) {
    ...
    if (filters.attendanceTypeId) {
      records = records.filter(
        (record) => record.attendanceTypeId === filters.attendanceTypeId
      );
    }
    ...
    if (filters.sessionType && filters.sessionType !== "all") {
      if (filters.sessionType === "scheduled") {
        ...
        records = records.filter(
          (record) =>
            record.attendanceTypeId !== null &&
            record.attendanceTypeId !== undefined
        );
  ...
  records.forEach((record) => {
    Object.entries(record.studentStatuses || {}).forEach(
